...
const SearchPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isFormulaSearch && source !== 'Local') {
      ...
      setApiError(t('search.formulaLocalOnly'))
      return
    }
            ...
            if (src === 'Local' && Array.isArray(item.key_properties)) {
              ...
              item.key_properties
                ...
                .forEach((kp: any) => addRow({
                  ...
                  formula: kp.material || '-',
                  type: t(`search.scType.${kp.superconductor_type || 'unknown'}`),
  ...
  }, [stage, mode, source, elementsKey, filtersKey, formulaQuery, page, retryTick, lang, t])
        ...
        <Box sx={{ display:'grid',gridTemplateColumns:'280px 1fr',gap:3,alignItems:'start',minWidth:0,overflow:'hidden',
          ...
          <Box component="aside" sx={{ minWidth:0, maxWidth:'100%', overflow:'hidden', p:2.5, bgcolor:'background.paper', borderRadius:4, border:'1px solid', borderColor:'divider', boxShadow:1 }}>
            ...
            <Box sx={{ display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:2 }}>
              {/* 数据来源 */}
              <Box>
                <FilterGroupLabel text={t('search.source')} />
                <Box sx={{ display:'flex',gap:1,mt:0.5,flexWrap:'wrap' }}>
                  {['Local','Alexandria','HTSC-2025','All'].map(s=>(
                    <Box key={s} component="button" onClick={()=>setSource(s)}
                      ...
                      {s==='All'?t('search.allSources'):s}
                    </Box>
                  ))}
                </Box>
                {source === 'All' && (
                  <Typography sx={{ fontSize:11,color:'text.disabled',mt:0.75 }}>{t('search.allSourcesHint')}</Typography>
                )}
              </Box>
              <Box sx={{ height:'1px',bgcolor:'divider' }} />
              {/* Formula 关键词 */}
              <Box title={supported.has('formula') ? undefined : t('search.unsupportedFilter')} sx={{ opacity: supported.has('formula') ? 1 : 0.45 }}>
                <FilterGroupLabel text={t('search.formulaKeyword')} active={groupActive.formula} disabled={!supported.has('formula')} />
                <Box sx={shellSx}>
                  <Box component="input" placeholder={t('search.formulaExample')} disabled={!supported.has('formula')}
                    value={filters.formula} onChange={e=>setFilters({...filters,formula:e.target.value})}
                    sx={{ ...bareInputSx, textAlign:'left', px:1.5 }} />
                </Box>
              </Box>
              {/* 数值范围 */}
              <RangeField label={t('search.representativeTc')} unit="K" active={groupActive.tc} disabled={!supported.has('tc')}
                lo={filters.tcMin} hi={filters.tcMax}
                onLo={v=>setFilters({...filters,tcMin:v})} onHi={v=>setFilters({...filters,tcMax:v})} />
              <RangeField label={t('search.pressure')} unit="GPa" active={groupActive.pressure} disabled={!supported.has('pressure')}
                lo={filters.pMin} hi={filters.pMax}
                onLo={v=>setFilters({...filters,pMin:v})} onHi={v=>setFilters({...filters,pMax:v})} />
              <RangeField label={t('search.year')} active={groupActive.year} disabled={!supported.has('year')}
                lo={filters.yMin} hi={filters.yMax}
                onLo={v=>setFilters({...filters,yMin:v})} onHi={v=>setFilters({...filters,yMax:v})} />
              {/* 空间群编号范围 */}
              <Box title={supported.has('space') ? undefined : t('search.unsupportedFilter')} sx={{ opacity: supported.has('space') ? 1 : 0.45 }}>
                <FilterGroupLabel text={t('search.spaceGroupNumber')} unit="#1–230" active={groupActive.space} disabled={!supported.has('space')} />
                <Box sx={shellSx}>
                  <Select variant="standard" disableUnderline disabled={!supported.has('space')} MenuProps={selectMenuProps}
                    ...
                    {[0,1,14,62,166,194,225].map(v=><MenuItem key={v} value={v}>{v === 0 ? t('search.unlimited') : v}</MenuItem>)}
                  </Select>
                  <Typography sx={{ color:'text.disabled',px:0.25,userSelect:'none' }}>–</Typography>
                  <Select variant="standard" disableUnderline disabled={!supported.has('space')} MenuProps={selectMenuProps}
                    value={filters.spaceMax} onChange={e=>setFilters({...filters,spaceMax:+e.target.value})} sx={selectSx}>
                    {[14,62,166,194,225,230].map(v=><MenuItem key={v} value={v}>{v === 230 ? t('search.unlimited') : v}</MenuItem>)}
                  </Select>
                </Box>
              </Box>
              <Box sx={{ height:'1px',bgcolor:'divider' }} />
              {/* 超导类型 */}
              <Box title={supported.has('type') ? undefined : t('search.unsupportedFilter')} sx={{ opacity: supported.has('type') ? 1 : 0.45 }}>
                <FilterGroupLabel text={t('search.superconductorType')} active={groupActive.type} disabled={!supported.has('type')} />
                <Box sx={shellSx}>
              ...
              {/* 审核状态 */}
              <Box title={supported.has('review') ? undefined : t('search.unsupportedFilter')} sx={{ opacity: supported.has('review') ? 1 : 0.45 }}>
                <FilterGroupLabel text={t('search.reviewStatus')} active={groupActive.review} disabled={!supported.has('review')} />
                <Box sx={shellSx}>
                  <Select variant="standard" disableUnderline disabled={!supported.has('review')} MenuProps={selectMenuProps}
                    ...
                    {[
                      ...
                      {v:'Rejected',l:t('search.status.rejected')},{v:'External',l:t('search.status.external')},
                    ].map(o=><MenuItem key={o.v} value={o.v}>{o.l}</MenuItem>)}
                  </Select>
                </Box>
              </Box>
              ...
              <Box component="button" disabled={!supported.has('chart')}
                title={supported.has('chart') ? undefined : t('search.unsupportedFilter')}
