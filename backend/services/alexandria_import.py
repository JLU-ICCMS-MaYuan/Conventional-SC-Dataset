...
# 仓库根目录下的 data/alexandria/（39GB 完整库仅供 download 端点使用，检索已迁 MySQL）
...
def query_by_elements(elements: List[str], mode: str = "contains",
                      ...
                      max_tc: float = None, pressure_min: float = None,
                      pressure_max: float = None, spg_min: int = None,
                      spg_max: int = None, require_tc: bool = False,
    ...
    pressure_sql = "(-(e.stress_xx + e.stress_yy + e.stress_zz) / 3.0 * 0.1)"
    stress_not_null = "e.stress_xx IS NOT NULL AND e.stress_yy IS NOT NULL AND e.stress_zz IS NOT NULL"
    filters = []
    fp = []
    ...
    if max_tc is not None:
        ...
        fp.append(max_tc)
    if require_tc:
        filters.append("(tc_max IS NOT NULL OR tc_allen_dynes IS NOT NULL)")
    if spg_min is not None:
        filters.append("e.spg >= ?")
        fp.append(spg_min)
    if spg_max is not None:
        filters.append("e.spg <= ?")
        fp.append(spg_max)
    if pressure_min is not None:
        filters.append(f"({stress_not_null} AND {pressure_sql} >= ?)")
        fp.append(pressure_min)
    if pressure_max is not None:
        filters.append(f"({stress_not_null} AND {pressure_sql} <= ?)")
        fp.append(pressure_max)
    if stable_only:
