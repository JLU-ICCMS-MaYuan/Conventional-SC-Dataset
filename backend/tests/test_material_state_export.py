...
def test_export_is_self_contained_and_json_serializable():
    ...
    record = models.PropertyRecord(id=1, record_key="tc-1", paper_id=1, paper_revision=1, material_state_id=1, module_id=1, record_type="predicted_tc", property_code="tc", definition_id=2, definition_key=record_definition.definition_key, definition_version=1, name_raw="Tc", value_kind="number", value_raw="250 K", value_number=250, canonical_unit="K", method_code="allen_dynes", is_representative=True, structure_key="structure-1", payload_json={"calculation_conditions": {}, "parameters": {"mu_star": {"value_number": 0.1, "unit": "1"}}}, source_fingerprint="b" * 64, record_checksum="c" * 64)
    ...
    assert payload["material_state"]["structures"][0]["structure_key"] == "structure-1"
...
def test_approved_export_rejects_record_without_evidence():
    engine = create_engine("sqlite://", connect_args={"check_same_thread": False}, poolclass=StaticPool)
    Base.metadata.create_all(engine)
    db = sessionmaker(bind=engine, expire_on_commit=False)()
    paper = models.Paper(id=1, title="Hg", year=1911, review_status="approved", content_revision=1, approved_revision=1, superconductor_kind="conventional")
    system = models.ChemicalSystem(id=1, paper_id=1, paper_revision=1, system_key="Hg", elements_list=["Hg"], element_count=1)
    material = models.Superconductor(id=1, paper_id=1, paper_revision=1, chemical_system_id=1, chemical_formula="Hg", formula_normalized="Hg", composition_key="Hg:1", display_name="Hg", elements_list=["Hg"], composition={"Hg": 1}, element_ratio={"Hg": 1})
    state = models.MaterialState(id=1, state_key="state-1", paper_id=1, paper_revision=1, superconductor_id=1, state_kind="experimental", crystal_system="unknown", material_dimensionality="three_dimensional")
    module_definition = _definition("module.superconductive_properties", "property_module")
    record_definition = _definition("record.superconductive_properties.measured_tc.resistivity", "property_record", record_type="measured_tc", method_code="resistivity", property_code="tc")
    module = models.PropertyModule(id=1, module_key="m1", paper_id=1, paper_revision=1, material_state_id=1, module_code="superconductive_properties", definition_key=module_definition.definition_key, definition_version=1, display_order=0, metadata_json={})
    record = models.PropertyRecord(id=1, record_key="tc-1", paper_id=1, paper_revision=1, material_state_id=1, module_id=1, record_type="measured_tc", property_code="tc", definition_id=2, definition_key=record_definition.definition_key, definition_version=1, name_raw="Tc", value_kind="number", value_raw="4.2 K", value_number=4.2, canonical_unit="K", method_code="resistivity", is_representative=True, payload_json={"experimental_conditions": {}}, source_fingerprint="b" * 64, record_checksum="c" * 64)
    db.add_all([paper, system, material, state, module_definition, record_definition, module, record])
    db.commit()

    with pytest.raises(HTTPException) as exc_info:
        build_material_state_export(db, paper_id=1, state_key="state-1", user=None)
    assert exc_info.value.status_code == 409
    assert exc_info.value.detail["code"] == "export_incomplete"
