...
本文档用于解释当前仓库中 `backend/api/tc_predict.py` 的业务逻辑、预测公式、输入输出、异常处理，以及该模块与主站数据库模块之间当前的关系和后续更合理的接入方向。

本文档关注的是“代码当前真实行为”，而不是理想化设计。

---

## 2. 模块在系统中的位置

当前 Tc 预测模块是主站中的一个独立实验功能。

- 页面入口：`/tc-pre`
- 前端页面模板：`frontend/templates/tc_pre.html`
- 前端交互脚本：`frontend/static/js/tc_pre.js`
- 后端 API：`backend/api/tc_predict.py`
- FastAPI 路由注册：`backend/main.py`
- 返回结果模型：`backend/schemas.py` 中的 `TcPredictionResponse`

从系统结构上看，它属于“挂在主站上的独立即时计算服务”，而不是“数据库驱动的业务子系统”。
...
1. 用户打开 `/tc-pre` 页面。
2. 页面要求上传一个结构文件 `CONTCAR/POSCAR`。
3. 页面要求上传一组 PDOS 文件，其中必须包含 `PDOS_H.dat`。
4. 浏览器通过 `FormData` 提交到 `POST /api/tc-predict`。
5. 后端在内存中解析结构和 PDOS 数据。
6. 后端提取 H 子晶格的键长分布，并从 PDOS 文件中读取费米能附近的态密度值。
7. 后端基于一组经验规则和线性映射计算 `predicted_tc`。
8. 后端返回 JSON。
9. 前端展示预测结果和若干中间特征量。
...
虽然 API 参数名叫 `contcar`，前端文案中也有 `POSCAR` 的描述，但本质上都依赖 VASP 的 POSCAR/CONTCAR 文本格式，由 `pymatgen.Structure.from_str(..., fmt="poscar")` 来解析。
...
这些文件提供的核心信息是费米能附近的投影态密度。

当前实现只用到了每个 PDOS 文件的一个标量：

- 取第一列作为能量 `energies`
- 取最后一列作为 DOS `dos_values`
- 找到最接近 0 eV 的能量点
- 读取该点的 DOS 值

因此，当前模块并没有利用完整 PDOS 曲线，只是从每个文件中抽取“费米能附近最后一列 DOS 值”作为输入特征。
...
- `BOND_THRESHOLD = 1.4`
  只把 `1.4 Å` 以内的 H-H 距离视为有效短键，参与后续统计。

- `COUPLING_RANGE = (0.98, 1.28)`
  只有落在 `0.98 Å` 到 `1.28 Å` 区间内的 H-H 键长才被计入耦合项。

- `SLOPE1` 和 `SLOPE2`
  最终使用线性关系：
  `predicted_tc = SLOPE1 * f2 + SLOPE2`
  它们是经验拟合参数，不是从本文件中推导出来的理论常数。

- `MAX_METAL_FILES = 4`
  最多读取 4 个非氢 PDOS 文件，超过部分直接忽略；不足 4 个则用 `0.0` 补齐。

这说明当前模型不是通用第一性原理求解器，而是一个基于结构特征和 DOS 特征的轻量经验预测器。
...
1. 把上传内容按 UTF-8 解码。
2. 交给 `pymatgen` 按 `poscar` 格式解析。
3. 解析失败则返回 400。

物理意义：

- 这一步的目标是获得完整晶体结构对象，后续所有几何分析都依赖它。
- 如果结构文件格式错了，整个预测无法进行，因为后续无法知道 H 原子怎么排布。

---

### 5.3 第二步：提取 H 子晶格与 H-H 键长分布
...
- `bonds`: 所有被判定为有效的 H-H 短键长度列表
- `h_atoms`: 结构中 H 原子总数
- `super_cell`: 估算出来的超胞倍数
...
- 从完整结构中删去所有非 H 原子，只保留 H 子晶格。

物理意义：

- 当前模型核心关注的是氢子晶格的短程构型。
- 金属原子在几何侧没有直接进入特征提取，而是在 DOS 侧间接体现。
...
- 模型假定短程 H-H 键是 Tc 预测中更关键的几何信息。
- 长于该阈值的 H-H 距离被视为对当前经验耦合贡献较弱，因此被截断。
...
- 用结构中的 H 原子总数除以最简化学式中的 H 数目。
- 结果必须是整数，否则认为结构文件与化学组成不一致。

物理意义：

- 如果结构是一个合法超胞，那么总 H 数应当等于“最简化学式的 H 数量 × 超胞倍数”。
- 这里的校验不是为了直接进入公式，而是为了保证输入结构的化学一致性。

---

### 5.4 第三步：从 PDOS 文件中取费米能态密度
