...
| 约束来源 | 强制要求 | 设计如何满足 | 状态 |
| --- | --- | --- | --- |
| Spec FR-001/FR-002 | 真实元数据与明确入口 | 管理端详情 DTO 计算用户名和当前版本物性数；页面分离物性记录与处理记录 | 待实施 |
| Spec FR-003/FR-004 | 仅管理员读取时间线 | 新增管理端专用 GET 路由与权限测试 | 待实施 |
| Spec FR-005 至 FR-009 | 单一、可信、可回填历史 | 表迁移、操作标识唯一约束、写入事务与不伪造修改规则 | 待实施 |
| Spec FR-010/FR-011 | 统计与删除一致 | 审核榜筛选 `reviewed`；删除拓扑加入新表 | 待实施 |
