...
分段和汇总只生成、校验并保存英文 canonical draft。前端审核表单直接使用这份值；证据组件仅为非空 `quote` 提供默认折叠的“论文片段”。删除任务创建、Redis、Worker、草稿 API、审核快照及前端类型中的建议副本字段；读取旧草稿时忽略它们。上传审核页同步去除“AI 草稿”命名。
...
| FR-008 至 FR-009 | `EvidenceNotes` 过滤与折叠展示 | Vitest 空证据和展开证据测试 |
| FR-010 | 上传页签、标题与状态 i18n | Vitest 中英文禁用词扫描 |

## 执行顺序

1. 删除建议副本生成、传输、类型和界面展示路径。
2. 将旧字段限定为读取兼容输入，并在新保存/提交快照中剔除。
3. 收敛证据渲染：过滤空 `quote`、默认折叠、标注为论文片段，并移除 AI 草稿命名。
4. 更新测试、Spec、Issue 与 Overview，清理 Pb 活动任务。
5. 运行后端测试、前端 Vitest 和生产构建。
