...
- [x] T002 [US1] [US2] 先在 tests/08_news/test_collection.py、tests/08_news/test_scheduler.py 编写解析、错误、事务去重及调度行为测试。
...
- [x] T006 [US3] 在 tests/08_news/NewsFeed.test.tsx 编写筛选分页、状态、重试及链接测试，更新 vitest.config.ts。
- [x] T007 [US3] 实现 frontend/src/components/NewsFeed.tsx 并接入 frontend/src/pages/NewsPage.tsx，保留人工内容及里程碑。

## 阶段 4：跨故事验证与收尾
- [x] T008 在 tests/08_news/ 补充隔离持久数据库/队列集成验证，执行前后端测试、构建及浏览器宽窄屏检查，在 quickstart.md 记录结果及边界。
...
- **后端测试**：16/16 通过（test_collection.py + test_scheduler.py），覆盖解析、去重、版本管理、限流、事务回滚
