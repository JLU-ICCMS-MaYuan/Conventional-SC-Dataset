...
**决策**：`tc_method` 在 API、草稿和数据库中始终使用英文稳定枚举；下拉框只从 i18n 枚举字典读取显示标签。

**理由**：将中文文案写入枚举值会破坏后端校验、历史记录和跨语言一致性。中文标签使用“方法”等中文说明，McMillan、Allen-Dynes、Eliashberg 与 SCDFT 等专有名词保留原文。
