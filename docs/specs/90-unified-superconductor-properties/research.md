...
**拒绝方案**：把计算、实验和所有方法字段都塞进一张宽表。Conditions 和方法参数作为当前记录的分组进入
受校验 JSON，避免大量固定空列。

**证据**：`backend/models.py` 的 `TcResult` 已有稳定查询列和专用约束，而
`SuperconductorProperty` 提供通用数值表达；目标模型需要同时保留这两类能力。

## 4. 决策：每条 Tc 自带完整条件与参数

**决策**：采用用户确认的图二。每条 Tc 自带结果、方法、Conditions、λ、ωlog、μ*、k/q 网格与展宽、
证据和补充信息。数据保存于当前记录的固定核心列与结构化 JSON 中，允许跨记录重复。

**理由**：λ、ωlog 本身会随网格、展宽等计算设置改变，作为当前 Tc 的完整计算资料保存更容易理解、
修改和导出。用户不需要维护额外参数记录、条件分组和输入配对关系。

**边界**：重复是不同结果的独立资料，复制后互不联动。每条记录内部仍有唯一的字段权威，不允许
固定 Tc 值在扩展字段中出现另一可写副本。独立报告的 λ 等可以另录普通物性，但不与 Tc 输入自动同步。

**取代的方案**：撤销共享 `condition_key`、独立输入 PropertyRecord、`identity_rules`、
`group_rules` 和两个 Conditions 外键。Conditions 名称保留，但它是记录内部对象，不新建对应实例表。

**代价**：有意增加少量重复数据；修订一个参数只影响当前记录，其他记录需分别审核修订。
换取每条结果可独立解释，以及按 MaterialState 下载完整资料的直接体验。

**证据**：用户明确选择“每条 Tc 自带完整资料，参数允许重复”，并以 λ、ωlog 对网格展宽的依赖解释原因。
...
区分模块和物性记录。JSON Schema 管当前记录的条件、参数和扩展分组，UI Schema 管控件和顺序；
内嵌资料随记录绑定同一版本，不再引入跨记录组规则。
...
和代表 Tc。参数及条件按旧来源到目标记录字段的映射展开核对，预期复制不等于迁移重复；无人引用的
旧条件和参数必须明确归属或归档后才能切换。缺失 Evidence 进入报告，不伪造。
...
**决策**：按 MaterialState 导出完整数据包，包含所属材料资料、状态、结构、各模块记录、条件和参数、
证据内容及定义版本。包内解析所有科学引用，不要求用户单独导关系表或联网查询定义。

用户在 Schema 预设分组中新增字段，分组位置决定字段归属；使用已发布的通用条目结构记录名称、
类型、单位、值和证据，随论文审核保留。全站新增规范字段仍需显式发布定义版本。

**理由**：直接保留用户填写时的组织结构，同时维持类型和审核边界。无需增加 Tc 输入关系表或为每个
用户字段建立独立 Schema 版本；这也不改变第 10 节的模块级自定义性质提升权限。

## 迁移审计资料移出业务库（2026-09-08）

用户明确选择迁移完成后删除三张临时表，审计内容保存在 #90。本决定取代数据库长期保留策略。
采用完整 Markdown 归档及本机 SQL 备份保存历史结构、记录和校验值，通过独立 Alembic 迁移清理业务库。
不删除历史迁移脚本，也不撤销迁移进行中的停写门；检查点缺失时正常放行。归档只解释迁移历史，不能替代
完整业务备份，后续编辑不回写归档。
