...
8. **Contract**：删除旧业务表或旧列；保留迁移检查点、逐条映射和异常清单作为只读审计元数据。该步骤
   使用独立迁移，不与复制步骤假定为同一事务。
...
Contract 后保留以下三张表，但它们不再参与正常业务读写：

| 表 | 保留内容 | Contract 后规则 |
| --- | --- | --- |
| `issue90_migration_checkpoint` | 最终阶段、读写切换、对账、观察结果和已删除旧表清单 | 固定为最终完成凭证 |
| `issue90_property_migration_map` | 来源表、旧 ID、论文 revision、目标 ID/record_key、字段映射和校验和 | 保留逐条来源追溯 |
| `issue90_migration_anomalies` | 迁移异常、原始详情和解决状态 | 即使为空也保留“无未解决异常”的可核验结构 |

应用的上传、编辑、审核、搜索、统计和导出路径不得依赖或更新这些表。未来若要退役，必须通过独立迁移先
生成包含表结构和全部记录的可核验归档，确认备份恢复、审计查询和旧 ID 排障不再依赖它们，再执行删除；
不能仅因当前异常表为空或迁移已经完成而手工删除。
