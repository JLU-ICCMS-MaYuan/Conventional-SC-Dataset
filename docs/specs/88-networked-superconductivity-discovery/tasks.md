...
- [ ] T002 [P] 扩展 `backend/news/domain.py`、`backend/news/models.py` 和 Go 模型，加入内容语义、展示栏位、发现来源、原文来源与命中依据。
- [ ] T003 [P] 新增 `alembic/versions/` 中的 #88 资讯字段迁移，并覆盖 SQLite/MySQL 契约。
- [ ] T004 修复 `backend/news/scheduler.py`、`backend/news/__main__.py` 的 Redis 长连接读超时和 Worker 自动恢复。

## 阶段 3：用户故事 1——发现最新超导文献（P1，MVP）

### 测试

- [ ] T005 [P] [US1] 为 `backend/news/sources.py` 增加 OpenAlex、官方来源注册和 Crossref/OpenAlex 补充发现的失败测试。

### 实施

- [ ] T006 [P] [US1] 实现 `backend/news/sources.py` 的 OpenAlex 和可配置出版社适配器，固定主题为 `superconduct`/`超导`。
- [ ] T007 [US1] 更新 `backend/news/service.py` 的身份去重和多来源链接合并，保留原文来源。

## 阶段 4：用户故事 2——科研与社会资讯（P1）

- [ ] T008 [P] [US2] 实现科研报道、社会/产业资讯适配器边界，并将 Phys.org 设置为期刊论文展示栏位。
- [ ] T009 [US2] 更新 `goserver/handlers/news_feed.go`、`frontend/src/components/NewsFeed.tsx` 和中英文资讯文案，展示内容语义与来源标签。

## 阶段 5：用户故事 4——追溯与验证（P2）

- [ ] T010 [P] [US4] 扩展 Go API、Python/Go/Vitest 测试，验证新字段、来源状态和安全外链。
