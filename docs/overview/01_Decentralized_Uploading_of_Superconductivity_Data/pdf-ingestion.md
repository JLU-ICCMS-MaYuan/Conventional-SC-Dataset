...
- 旧顶层 `sc_type` 和状态级 `material_family` 只在草稿 GET/内部 AI 归一化时一次性提升为论文级列表；客户端 PUT 和 submit 返回 `legacy_classification_contract`，不再接受状态级写入。
