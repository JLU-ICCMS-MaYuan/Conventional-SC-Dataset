...
- 筛选条件各自对应真实列：Tc 用 `tc_results.tc_value_k`，压强用 `material_states.pressure_value_gpa`，超导类型用 `material_states.state_kind`，元素用 `material_states.superconductor_id`，关键词匹配论文字段或 `superconductors.chemical_formula`。（[Issue #57](https://github.com/JLU-ICCMS-MaYuan/SC-Wiki/issues/57)）
