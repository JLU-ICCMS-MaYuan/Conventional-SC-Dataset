...
- 图表组合 API 支持列表、详情、创建、更新、删除和公开状态切换；组合点可以引用物性记录或使用自定义点字段。组合点引用的压强与超导类型取自所属材料状态，不再取自物性记录本身（物性表无这两列）。（[Issue #57](https://github.com/JLU-ICCMS-MaYuan/SC-Wiki/issues/57)）
