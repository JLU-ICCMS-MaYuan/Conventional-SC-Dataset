...
\begin{abstract}
随着超导材料研究的快速发展，研究者对超导相关信息的系统化组织、可追溯管理与高效获取提出了更高要求。现有公共材料数据库和近年构建的超导数据集已在结构--性质关联分析、基准测试构建和高通量筛选等方面发挥重要作用；然而，在人工智能驱动材料发现的背景下，超导研究还需要能够持续整合文献证据、结构信息、物理参数和计算资源的数据基础设施，以支持候选体系识别、数据质量控制以及机器学习模型训练与评估。为此，本文设计并实现了 Conventional-SC-Dataset，一个面向常规超导体研究的结构化数据管理与共享平台。该平台以超导体系和元素组合作为组织主线，系统整合体系名称、晶体结构与对称性、压力条件、电子结构特征、电子--声子耦合相关参数以及超导临界转变温度等关键物理属性，并建立与来源文献相连接的可追溯记录机制。平台支持基于周期表的元素导航、三种标准化超导体系检索模式、AI 辅助文献筛选与批量数据导入，同时集成 HTSC-2025 基准数据浏览、Alexandria 电声耦合数据接口，以及关联结构文件和投影态密度（PDOS）信息的 Tc 预测模块，从而形成覆盖数据收集、管理、查询、审核与分析的完整工作流程。当前平台主数据库覆盖元素周期表全部 118 种元素；配套 AI 筛选数据库已收录 688 篇文献、1,010 种化合物和 2,624 条物理参数记录，其中 1,921 条记录包含超导转变温度信息，2,068 条记录包含压力信息；系统还提供包含 140 条标准化记录的 HTSC-2025 基准数据集入口。本文工作的目标并非提出新的超导机理或验证新的预测模型，而是构建一个可复现、可扩展并支持协同维护的数据基础设施，将分散于文献和相关计算资源中的超导数据整合为面向元素组织、具备可追溯性与可复用性的结构化知识资源，为超导材料的数据驱动研究提供支撑。
\end{abstract}
